...
from ultralytics import YOLO # <-- Kembali ke pustaka utama
import subprocess
...
def load_model():
    ...
    Ini adalah cara yang benar untuk model YOLOv8.
    """
    model_path = "yolov8_hardhat.pt"
    # Menggunakan model YOLOv8 yang telah terbukti berfungsi
    ...
    if not os.path.exists(model_path):
        ...
        try:
            # Mengunduh bobot model dari Hugging Face
            hf_hub_download(
                repo_id=repo_id,
                filename=filename,
                local_dir='.',
    ...
    try:
        # ===== KEMBALI KE LOGIKA ASLI YOLOv8 =====
        # Langsung memuat model dari file .pt menggunakan kelas YOLO
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with st.spinner("Sedang memproses video... Harap tunggu."):
                    ...
                    for i, frame_path in enumerate(frame_files):
                        # ===== PERUBAHAN LOGIKA DETEKSI UNTUK YOLOv8 =====
                        # Tidak perlu konversi warna, model dipanggil langsung
                        results = model(str(frame_path), verbose=False)
                        
                        # Visualisasi dengan results[0].plot() yang merupakan metode bawaan YOLOv8
